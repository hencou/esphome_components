...
void Remeha::process_trending_data_() {
  ...
  // Room temperature: byte 71, single byte × 0.01
  if (len > 71 && this->room_temperature_ != nullptr) {
    float room_temp = d[71] * 0.1f;
  ...
  if (this->climate_ != nullptr && len > 71) {
    float room_temp2 = d[71] * 0.1f;
