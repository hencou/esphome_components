#include "i2c_esp32_bus_esp_idf.h"
...
static const char *const TAG = "i2c_esp32.idf";
...
void IDFI2CBus::init(uint8_t address, i2c_mode_t mode) {
  ...
  conf.sda_pullup_en = (gpio_pullup_t)sda_pullup_enabled_;
  conf.scl_io_num = (gpio_num_t)scl_pin_;
  conf.scl_pullup_en = (gpio_pullup_t)scl_pullup_enabled_;
  if (mode == I2C_MODE_SLAVE) {
    conf.slave = {0, address};
  } else {
  ...
  if (mode == I2C_MODE_SLAVE) {
    err = i2c_driver_install(port_, I2C_MODE_SLAVE, 512, 0, ESP_INTR_FLAG_IRAM);
  } else {
...
ErrorCode IDFI2CBus::write_bytes_raw(const char* buffer, uint32_t len) {
  init(0x00, I2C_MODE_MASTER);
  // logging is only enabled with vv level, if warnings are shown the caller
  // should log them
  if (!initialized_) {
  ...
  esp_err_t err = i2c_master_start(cmd);
  if (err != ESP_OK) {
    ESP_LOGVV(TAG, "RX from master start failed: %s", esp_err_to_name(err));
    i2c_cmd_link_delete(cmd);
    return ERROR_UNKNOWN;
  }
  err = i2c_master_write(cmd, (uint8_t*)buffer, len, true);
  if (err != ESP_OK) {
    ESP_LOGVV(TAG, "RX from address write failed: %s", esp_err_to_name(err));
    i2c_cmd_link_delete(cmd);
    return ERROR_UNKNOWN;
  }
  err = i2c_master_stop(cmd);
  if (err != ESP_OK) {
    ESP_LOGVV(TAG, "RX from stop failed: %s", esp_err_to_name(err));
    i2c_cmd_link_delete(cmd);
    return ERROR_UNKNOWN;
  }
  err = i2c_master_cmd_begin(port_, cmd, 20 / portTICK_PERIOD_MS);
  ...
  if (err == ESP_FAIL) {
    // transfer not acked
    ESP_LOGVV(TAG, "RX from failed: not acked");
    return ERROR_NOT_ACKNOWLEDGED;
  } else if (err == ESP_ERR_TIMEOUT) {
    ESP_LOGVV(TAG, "RX from failed: timeout");
    return ERROR_TIMEOUT;
  } else if (err != ESP_OK) {
    ESP_LOGVV(TAG, "RX from failed: %s", esp_err_to_name(err));
    return ERROR_UNKNOWN;
  }
...
size_t IDFI2CBus::slave_receive(uint8_t address, uint8_t receive_buf[]) {
  init(address, I2C_MODE_SLAVE);
  // logging is only enabled with vv level, if warnings are shown the caller
  // should log them
  if (!initialized_) {
    ESP_LOGVV(TAG, "i2c bus not initialized!");
  ...
  while (1) {
    int len1 = i2c_slave_read_buffer(port_, i2cbuf + buflen, sizeof(i2cbuf) - buflen, 50 / portTICK_RATE_MS );
    if (len1 <= 0)
      break;
    buflen += len1;
  }
...
ErrorCode IDFI2CBus::writev(uint8_t address, WriteBuffer *buffers, size_t cnt, bool stop) {
  ...
  for (size_t i = 0; i < cnt; i++) {
    ...
    if (buf.len == 0)
      continue;
    err = i2c_master_write(cmd, const_cast<uint8_t*>(buf.data), buf.len, true);
    if (err != ESP_OK) {
