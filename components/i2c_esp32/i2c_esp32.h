...
class I2CDevice {
 public:
  ...
  ErrorCode write_bytes_raw(const char* buf, uint32_t len) { return bus_->write_bytes_raw(buf, len); }
  size_t slave_receive(uint8_t i2c_receive_buf[])  { return bus_->slave_receive(address_, i2c_receive_buf); }
